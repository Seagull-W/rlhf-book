...
For an RTX 4060-class GPU, use the maintained LoRA configuration. It freezes
the bf16 base model and keeps only the adapters in fp32, preserves assistant
end-of-turn tokens, evaluates on a fixed validation subset, and uses
deterministic sample generation.

~~~bash
HF_ENDPOINT=https://huggingface.co uv run python -m instruction_tuning.train \
    --config instruction_tuning/configs/sft_qwen2_5_0_5b_lora_rtx4060_8gb.yaml \
    --device cuda
~~~

The configuration writes per-step metrics and complete generated samples to
the paths declared by metrics_log_path and samples_log_path, and saves the
final PEFT adapter plus tokenizer under output_dir. Use tee if a complete
console transcript is also desired.
